...
archivos <- list.files('Data/Sheets',pattern = '.xlsx$',recursive = T,full.names = T)

nombres <- list.files('Data/Sheets',pattern = '.xlsx$',recursive = T,full.names = F)
...
for(j in 1:length(archivos)){
  ...
  print({plot(x=0,y=0, type = 'n',
    ...
    plot(as(krigging[[1]],'RasterLayer'), add=T,
    ...
    points(db,pch=20)
    contour(krigging[[1]],add=T,col='black',lwd=0.3,drawlabels = F)
