...
if (sum(is.na(as.numeric(c(hoja[7,3],hoja[8,3])))) == 0) {
  ...
  #flist[[i]][[11]] <- north2(map2,0.8,0.8, scale = 0.2)
}else{
  par(mar = c(0,0,0,0))
...
x_test <- seq(from = 0,to = maxRef,by = 0.01)
...
for (i in seq_along(patt)) {
  ...
  flist[[i]][[12]] %>% kable(digits = 2, format = 'latex', booktabs = T,linesep = "", align = 'c', caption = paste0('Tabla de altura - caudal del canal  ',flist[[i]][[5]]),row.names = F, format.args = list(decimal.mark = ',', big.mark = ".")) %>% kable_styling(latex_options = c("repeat_header")) %>% row_spec(0, bold = T) %>% print()
